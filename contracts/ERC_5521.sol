...
contract ERC_5521 is ERC721, ERC721URIStorage, IERC_5521, TargetContract {
    ...
    function setTokenURI(uint256 tokenId, string memory _tokenURI) public {
        ...
        require(
            ...
            "ERC_5521: transfer caller is not owner nor approved"
        );
    ...
    function setNode(
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            if (tokenIds[i].length == 0) {
                revert("ERC_5521: the referring list cannot be empty");
            }
        }
    ...
    function setNodeReferring(
        ...
        for (uint i = 0; i < addresses.length; i++) {
            if (relationship.referring[addresses[i]].length == 0) {
        ...
        for (uint i = 0; i < addresses.length; i++) {
            if (addresses[i] == address(this)) {
                for (uint j = 0; j < _tokenIds[i].length; j++) {
                    Relationship storage relationship = _relationship[
                        _tokenIds[i][j]
                    ];
                    if (relationship.referred[addresses[i]].length == 0) {
                        relationship.referredKeys.push(addresses[i]);
                    } // Add the address if it's a new entry

                    require(
                        tokenId != _tokenIds[i][j],
                        "ERC_5521: self-reference not allowed"
                    );
                    if (relationship.createdTimestamp >= block.timestamp) {
                        revert(
                            "ERC_5521: the referred rNFT needs to be a predecessor"
                        );
                ...
                TargetContract targetContractInstance = TargetContract(
                    addresses[i]
                );
                bool isSupports = targetContractInstance.supportsInterface(
                    type(TargetContract).interfaceId
                );
                if (isSupports) {
                    // The target contract supports the interface, safe to call functions of the interface.
                    targetContractInstance.setNodeReferredExternal(
                        address(this),
                        tokenId,
                        _tokenIds[i]
                    );
    ...
    function setNodeReferredExternal(
        ...
        for (uint i = 0; i < _tokenIds.length; i++) {
            ...
            require(
                ...
                "ERC_5521: this must be an external contract address"
            );
            if (relationship.createdTimestamp >= block.timestamp) {
                revert("ERC_5521: the referred rNFT needs to be a predecessor");
            } // Make sure the reference complies with the timing sequence
        ...
        returns (address[] memory, uint256[][] memory)
    {
        ...
        if (_address == address(this)) {
            ...
            require(
                targetContractInstance.supportsInterface(
                    type(TargetContract).interfaceId
                ),
                "ERC_5521: target contract not supported"
            );
            (_referringKeys, _referringValues) = targetContractInstance
                .referringOf(_address, tokenId);
        }
        ...
        returns (address[] memory, uint256[][] memory)
    {
        ...
        if (_address == address(this)) {
            ...
            (_referredKeys, _referredValues) = targetContractInstance
                .referredOf(_address, tokenId);
        }
    ...
    function createdTimestampOf(
        address _address,
        uint256 tokenId
    ) external view override(IERC_5521, TargetContract) returns (uint256) {
        ...
        if (_address == address(this)) {
            ...
            createdTimestamp = targetContractInstance.createdTimestampOf(
                _address,
                tokenId
            );
        }
    ...
    {
        return
            interfaceId == type(IERC_5521).interfaceId ||
    ...
    function emitEvents(uint256 tokenId, address sender) private {
        (
            address[] memory _referringKeys,
            uint256[][] memory _referringValues
        ) = convertMap(tokenId, true);
        (
            address[] memory _referredKeys,
            uint256[][] memory _referredValues
        ) = convertMap(tokenId, false);
